...
router.post("/subCategory/addSubCategory", async (req, res) => {
  try {
    const { title, main_category_id, image_url } = req.body;

    // Execute the query using the established pool
    const newSubCategory = await pool.query(
      "INSERT INTO sub_category (title, main_category_id, image_url) VALUES ($1, $2, $3) RETURNING id, title, main_category_id, image_url",
      [title, main_category_id, image_url]
    );
...
router.get("/subCategory/allSubCategories", async (req, res) => {
  try {
    const allSubCategories = await pool.query(
      "SELECT id, title, main_category_id, image_url FROM sub_category"
    );
...
router.get("/subCategory/subCategories/:id", async (req, res) => {
  try {
    ...
    const searchSubCategory = await pool.query(
      "SELECT id, title, main_category_id, image_url FROM sub_category WHERE id = $1",
      [id]
    );
...
router.put("/subCategory/updateSubCategory/:id", async (req, res) => {
  try {
    ...
    const { title, main_category_id, image_url } = req.body; // Extract individual fields from req.body

    const updateCategory = await pool.query(
      "UPDATE sub_category SET title = $1, main_category_id = $2, image_url = $3 WHERE id =$4",
      [title, main_category_id, image_url, id] // Add id to the parameter list
    );
